#!/bin/bash

echo
echo ":: >> >> >> >> >> >> >> >>"
echo ":: >> Enable armbian beta"

echo ":: Import armbian key"
apt-key add - <<'_ARMBIAN_KEY_'
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v1


=I/P8
-----END PGP PUBLIC KEY BLOCK-----
_ARMBIAN_KEY_

echo ":: Add armbian beta sources"
cat > /etc/apt/sources.list.d/armbian-beta.list << _ARMBIAN_LIST_
deb http://beta.armbian.com/ $(lsb_release -s --codename) main
deb http://beta.armbian.com/ $(lsb_release -s --codename) utils
deb http://beta.armbian.com/ $(lsb_release -s --codename) $(lsb_release -s --codename)-utils
deb http://beta.armbian.com/ $(lsb_release -s --codename) $(lsb_release -s --codename)-desktop
_ARMBIAN_LIST_

echo ":: Update packages"
apt-get update
apt-get -y dist-upgrade

echo ":: << Enable armbian beta"
echo ":: << << << << << << << <<"
echo
